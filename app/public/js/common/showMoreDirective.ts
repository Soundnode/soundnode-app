app.directive('textCollapse', ['$compile', function ($compile) {
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      const maxHeight = attrs.textMaxHeight;
      ...
      attrs.$observe('textToCollapse', function (text) {
        ...
        if (element.clientHeight > maxHeight) {
          const toggleButton = $compile('<button class="button show_more" ng-click="toggle()">{{collapsed ? "Show more" : "Show less"}}</button>')(scope);

...
app.controller('ChartsCtrl', function (
    ...
    const url_genre = $stateParams.genre;
    let genre = {};
    if(!url_genre){
        genre = {
            "link" : "all-music",
    ...
    function filterCollection(data) {
        return data.collection.filter(function (item) {
            ...
            const isTrackType = item.type === 'track' ||
            ...
            const exists = tracksIds.indexOf(item.track.id) > -1;
            if (exists) {
                return false;
            }
    ...
    function loadTracksInfo(collection) {
        const ids = collection.map(function (item) {

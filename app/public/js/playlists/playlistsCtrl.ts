...
app.controller('PlaylistsCtrl', function (
    ...
    $scope.removeFromPlaylist = function(songId, playlistId) {
        const endpoint = 'users/'+  $rootScope.userId + '/playlists/'+ playlistId;
        const params = '';

        SCapiService.get(endpoint, params)
            .then(function(response) {
                const uri = response.uri + '.json?&oauth_token=' + $window.scAccessToken;
                ...
                $http.put(uri, { "playlist": {
                    ...
                    const inQueue = queueService.find(songId);
